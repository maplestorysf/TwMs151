...
public class WizerDual {
    ...
    public static void monsterBomb(MapleCharacter player, MapleCharacter attackedPlayers, MapleMap map, AttackInfo attack) {
        ...
        if (mguard != null) {
            ...
            if (mploss > attackedPlayers.getStat().getMp()) {
                ...
                stats.put(MapleStat.MP, attackedPlayers.getStat().getMp());
        ...
        if (!attackedPlayers.wantHit()) { // GM Perk - Godmode
            ...
            attackedPlayers.getClient().getSession().write(CWvsContext.serverNotice(5, player.getName() + "攻擊傷害為０!"));
            return;
        }
        ...
        attackedPlayers.getClient().getSession().write(CWvsContext.serverNotice(5, player.getName() + " 已經打了 " + (damageCount * effect.getAttackCount()) + " 攻擊傷害!"));
        ...
        if (attackedPlayers.getStat().getHp() <= 0 && !attackedPlayers.isAlive()) {
            ...
            player.getClient().getSession().write(CWvsContext.serverNotice(6, "您已經殺了 " + attackedPlayers.getName() + " 且得到了殺人數!"));
    ...
    public static void doPvP(MapleCharacter player, MapleMap map, AttackInfo attack) {
        ...
        getDirection(player);
        player.getMap().getNearestPvpChar(player.getPosition(), maxDis, maxHeight, Collections.unmodifiableCollection(player.getMap().getCharacters())).stream().filter((attackedPlayers) -> (attackedPlayers.isAlive() && (player.getParty() == null || player.getParty() != attackedPlayers.getParty()))).forEachOrdered((attackedPlayers) -> {
    ...
    public static void doGuildPvP(MapleCharacter player, MapleMap map, AttackInfo attack) {
        DamageBalancer(attack);
        getDirection(player);
        player.getMap().getNearestPvpChar(player.getPosition(), maxDis, maxHeight, player.getMap().getCharacters()).stream().filter((attackedPlayers) -> (attackedPlayers.isAlive() && (player.getGuildId() != attackedPlayers.getGuildId()))).forEachOrdered((attackedPlayers) -> {
            monsterBomb(player, attackedPlayers, map, attack);
        });
    }
    ...
    public static void doRacistPvP(MapleCharacter player, MapleMap map, AttackInfo attack) {
        DamageBalancer(attack);
        getDirection(player);
        player.getMap().getNearestPvpChar(player.getPosition(), maxDis, maxHeight, player.getMap().getCharacters()).stream().filter((attackedPlayers) -> (attackedPlayers.isAlive() && (player.getSkinColor() != attackedPlayers.getSkinColor()))).forEachOrdered((attackedPlayers) -> {
            monsterBomb(player, attackedPlayers, map, attack);
        });
    }
    ...
    public static void doOccPvP(MapleCharacter player, MapleMap map, AttackInfo attack) {
        DamageBalancer(attack);
        getDirection(player);
        player.getMap().getNearestPvpChar(player.getPosition(), maxDis, maxHeight, player.getMap().getCharacters()).stream().filter((attackedPlayers) -> (attackedPlayers.isAlive() && (player.getOccId() != attackedPlayers.getOccId()))).forEachOrdered((attackedPlayers) -> {
            monsterBomb(player, attackedPlayers, map, attack);
        });
    }
    ...
    public static void doJobPvP(MapleCharacter player, MapleMap map, AttackInfo attack) {
        DamageBalancer(attack);
        getDirection(player);
        player.getMap().getNearestPvpChar(player.getPosition(), maxDis, maxHeight, player.getMap().getCharacters()).stream().filter((attackedPlayers) -> (attackedPlayers.isAlive() && (player.getJobId() != attackedPlayers.getJobId()))).forEachOrdered((attackedPlayers) -> {
            monsterBomb(player, attackedPlayers, map, attack);
        });
    }
    ...
    public static void doSurvivalPvP(MapleCharacter player, MapleMap map, AttackInfo attack) {
        DamageBalancer(attack);
        getDirection(player);
        player.getMap().getNearestPvpChar(player.getPosition(), maxDis, maxHeight, player.getMap().getCharacters()).stream().filter((attackedPlayers) -> (attackedPlayers.isAlive())).forEachOrdered((attackedPlayers) -> {
            monsterBomb(player, attackedPlayers, map, attack);
        });
    }
    ...
    public static void doGenderPvP(MapleCharacter player, MapleMap map, AttackInfo attack) {
        DamageBalancer(attack);
        getDirection(player);
        player.getMap().getNearestPvpChar(player.getPosition(), maxDis, maxHeight, player.getMap().getCharacters()).stream().filter((attackedPlayers) -> (attackedPlayers.isAlive() && (player.getGender() != attackedPlayers.getGender()))).forEachOrdered((attackedPlayers) -> {
            monsterBomb(player, attackedPlayers, map, attack);
        });
    }
    ...
    public static void doPartyPvP(MapleCharacter player, MapleMap map, AttackInfo attack) {
        DamageBalancer(attack);
        getDirection(player);
        player.getMap().getNearestPvpChar(player.getPosition(), maxDis, maxHeight, player.getMap().getCharacters()).stream().filter((attackedPlayers) -> (attackedPlayers.isAlive() && (player.getParty() != attackedPlayers.getParty()))).forEachOrdered((attackedPlayers) -> {
            monsterBomb(player, attackedPlayers, map, attack);
        });
    }
