...
public class PlayerStats implements Serializable {
    ...
    public void setStr(int st, MapleCharacter cf) {
        setStr((short) st, cf);
    }

    public void setDex(int de, MapleCharacter cf) {
        setDex((short) de, cf);
    }

    public void setInt(int in, MapleCharacter cf) {
        setInt((short) in, cf);
    }

    public void setLuk(int l, MapleCharacter cf) {
        setLuk((short) l, cf);
    }
    ...
    public void recalcLocalStats(boolean first_login, MapleCharacter chra) {
        ...
        while (itera.hasNext()) {
            ...
            if (equip.getIncSkill() > 0 && ii.getEquipSkills(equip.getItemId()) != null) {
                ii.getEquipSkills(equip.getItemId()).stream().map((zzz) -> SkillFactory.getSkill(zzz)).filter((skil) -> (skil != null && skil.canBeLearnedBy(chra.getJob()))).forEachOrdered((skil) -> {
                    //dont go over masterlevel :D
                    int value = 1;
        ...
        chra.getInventory(MapleInventoryType.CASH).newList().forEach((item) -> {
        ...
        chra.getCharacterCard().getCardEffects().stream().map((ix) -> SkillFactory.getSkill(((Integer) ix.getLeft())).getEffect(((Integer) ix.getRight()))).map((e) -> {
        ...
        if (chra.getGuildId() > 0) {
            ...
            if (g != null && g.getSkills().size() > 0) {
                ...
                g.getSkills().stream().filter((gs) -> (gs.timestamp > now && gs.activator.length() > 0)).map((gs) -> SkillFactory.getSkill(gs.skillID).getEffect(gs.level)).map((e) -> {
    ...
    public boolean checkEquipLevels(final MapleCharacter chr, long expgain) {
        long gain = expgain;
        ...
        for (Equip eq : all) {
            ...
            eq.setItemEXP(eq.getItemEXP() + (int) gain);
                    ...
                    if (GameConstants.getStatFromWeapon(eq.getItemId()) == null && GameConstants.getMaxLevel(eq.getItemId()) < (lvlz + i) && Math.random() < 0.1 && eq.getIncSkill() <= 0 && ii.getEquipSkills(eq.getItemId()) != null) {
                        for (int zzz : ii.getEquipSkills(eq.getItemId())) {
                            ...
                            if (skil != null && skil.canBeLearnedBy(chr.getJob())) { //dont go over masterlevel :D
                                ...
                                chr.dropMessage(5, "您的技能: " + skil.getName() + " 已經升級");
                            }
                        }
                    }
                    if (inc == null) {
                        eq = ii.levelUpEquipCustom(eq, lvlz + i);
                    }
                    chr.dropMessage(MessageType.ERROR, "裝備升級 [" + ii.getName(eq.getItemId()) + "] 等級 : (Level " + (eq.getEquipLevel() == 50 ? "最高" : eq.getEquipLevel()) + ").");
                }
                changed = true;
            }
    ...
    public boolean checkEquipDurabilitys(MapleCharacter chr, int gain, boolean aboveZero) {
        ...
        all.stream().filter((item) -> (item != null && ((item.getPosition() >= 0) == aboveZero))).map((item) -> {
            item.setDurability(item.getDurability() + gain);
            return item;
        }).filter((item) -> (item.getDurability() < 0)).forEachOrdered((item) -> {
