...
//            return new StringBuilder().append("!修改人氣商品 <第X個人氣商品> <新商品的SN> - 修改商城右邊人氣商品").toString();
//        }
//    }
                ...
                if (rs.next()) {
                    if (c.getPlayer().getGuildId() > 0) {
                        try {
                            ...
                            c.sendPacket(CWvsContext.getErrorNotice("無法連接到世界伺服器，請稍後再嘗試。"));
                            return false;
                        }
...
//    }
            ...
            if (minutesLeft > 0) {
                ts = EventTimer.getInstance().schedule(new Runnable() {
                    ...
                    public void run() {
                        ...
                        World.Broadcast.broadcastMessage(CWvsContext.getItemNotice("關鍵時刻開放囉，沒有30等以上的玩家是得不到的。"));
                        ts.cancel(false);
                        ts = null;
                    }
    ...
    public static class autoreg extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String splitted[]) {
            ...
            c.getPlayer().dropMessage(0, "[autoreg] " + (ServerConstants.getAR() ? "開啟" : "關閉"));
            System.out.println("[autoreg] " + (ServerConstants.getAR() ? "開啟" : "關閉"));
            return true;
        }
    ...
    public static class logindoor extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String splitted[]) {
            ...
            c.getPlayer().dropMessage(0, "[logindoor] " + (LoginServer.isAdminOnly() ? "開啟" : "關閉"));
            System.out.println("[logindoor] " + (LoginServer.isAdminOnly() ? "開啟" : "關閉"));
            return true;
        }
...
//            }
    ...
    public static class 查詢洗道具 extends CommandExecute {
        ...
        @Override
        public boolean execute(MapleClient c, String[] splitted) {
            c.getPlayer().dropMessage("開始查詢複製裝備....");
            OnlyID.getInstance().StartCheckings();
            c.getPlayer().dropMessage("複製裝備查詢完畢");
            return true;
        }

        @Override
        public String getMessage() {
            return new StringBuilder().append("!查詢洗道具 - 查詢洗道具").toString();
        }
    }
    ...
    public static class 處理洗道具 extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            if (OnlyIDList.isEmpty()) {
                c.getPlayer().dropMessage("目前沒有複製裝備的資料，請輸入 !查詢洗道具 來獲得資料");
                return true;
            } else {
                c.getPlayer().dropMessage(6, "正在開始處理複製裝備...");
            }
            try {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append("!處理洗道具 - 查詢洗道具").toString();
        }
        ...
        public void HandleOffline(MapleClient c, int chr, long inventoryitemid, long equiponlyid) {
            try {
                ...
                World.Broadcast.broadcastGMMessage(CWvsContext.getItemNotice("[GM密語] " + msgtext));
                FileoutputUtil.logToFile("logs/Hack/複製裝備_已刪除.txt", msgtext + " 道具唯一ID: " + equiponlyid + "\r\n");
