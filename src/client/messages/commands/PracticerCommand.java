...
public class PracticerCommand {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            if (victim != null) {
                if (splitted.length == 2) {
                    ...
                    try {
                        target = ChannelServer.getInstance(c.getWorld(), c.getChannel()).getMapFactory().getMap(Integer.parseInt(splitted[2]));
                    } catch (Exception ex) {
                ...
                if (ch < 0) {
                    ...
                    victim = ChannelServer.getInstance(c.getWorld(), ch).getPlayerStorage().getCharacterByName(input);
                    if (victim != null) {
    ...
    public static class WarpID extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            if (victim != null) {
                ...
                try {
                    try {
                        victim = ChannelServer.getInstance(c.getWorld(), ch).getPlayerStorage().getCharacterById(Integer.parseInt(splitted[1]));
                    } catch (Exception e) {
    ...
    public static class Ban extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            if (ch >= 1) {
                target = ChannelServer.getInstance(c.getWorld(), ch).getPlayerStorage().getCharacterByName(name);
            } else {
            ...
            World.Broadcast.broadcastMessage(CWvsContext.getItemNotice("[封鎖系統] " + splitted[1] + " 因為" + reason + "而被管理員永久停權。"));
    ...
    public static class BanID extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            String name = c.getPlayer().getCharacterNameById(c.getWorld(), id);
            target = ChannelServer.getInstance(c.getWorld(), ch).getPlayerStorage().getCharacterById(id);
            if (target == null) {
            ...
            FileoutputUtil.logToFile("logs/Hack/指令封鎖名單.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().getRemoteAddress().toString().split(":")[0] + " " + c.getPlayer().getName() + " 封鎖了 " + name + " 原因: " + sb.toString() + " 是否離線封鎖: " + offline);
            String reason = "null".equals(input) ? "使用違法程式練功" : StringUtil.joinStringFrom(splitted, 2);
            World.Broadcast.broadcastMessage(CWvsContext.getItemNotice("[封鎖系統] " + name + " 因為" + reason + "而被管理員永久停權。"));
    ...
    public static class CnGM extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            World.Broadcast.broadcastGMMessage(CWvsContext.getErrorNotice("<GM聊天視窗>" + "頻道" + c.getPlayer().getClient().getChannel() + " [" + c.getPlayer().getName() + "](" + c.getPlayer().getId() + ") : " + StringUtil.joinStringFrom(splitted, 1)));
            return true;
        }
    ...
    public static class online extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            totalOnline = ChannelServer.getAllInstances().stream().map((cserv) -> cserv.getConnectedClients()).reduce(totalOnline, Integer::sum);
        ...
        public boolean execute(MapleClient c, String splitted[]) {
            ...
            if (victim != null) {
                ...
                if (ch < 0) {
                    ...
                    victim = ChannelServer.getInstance(c.getWorld(), ch).getPlayerStorage().getCharacterByName(splitted[1]);
    ...
    public static class UnBan extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            if (ret_ == 1 || ret_ == 2) {
                FileoutputUtil.logToFile("logs/Hack/ban/解除封鎖名單.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().getRemoteAddress().toString().split(":")[0] + " " + c.getPlayer().getName() + " 解鎖了 " + splitted[1]
                );
            }
            return true;
        }
    ...
    public static class DC2 extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append("!dc2 <玩家> - 讓玩家-2147斷線").toString();
        }
    }
    ...
    public static class BanStatus extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append("!BanStatus <玩家名稱> <npc> - 查看玩家是否被封鎖及原因").toString();
        }
    }
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            try {
                ChannelServer.getAllInstances().forEach((cs) -> {
                    cs.getPlayerStorage().getAllCharactersThreadSafe().stream().filter((chr) -> (chr.getClient().getSessionIPAddress().equals(IP))).filter((chr) -> (!chr.getClient().isGm())).map((chr) -> {
