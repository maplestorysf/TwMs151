        ...
        switch (entry) {
            ...
            case Drop_Party_15: // exp rate + 100% party 30 min
                ...
                if (c.getPlayer().getParty() != null) {
                    c.getPlayer().getParty().getMembers().stream().filter((mpc) -> (mpc.getId() != c.getPlayer().getId())).map((mpc) -> c.getPlayer().getMap().getCharacterById(mpc.getId())).filter((chr) -> (chr != null)).forEachOrdered((chr) -> {
                        entry.applyTo(chr);
