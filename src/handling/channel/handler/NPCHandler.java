        ...
        switch (action) {
            ...
            case 1: { // Start Quest
                ...
                if (!q.hasStartScript()) {
                    if (chr.getDebugMessage()) {
                        chr.dropMessage(-1, "開始客戶端任務 NPC: " + npc + " Quest: " + quest);
                    }
                    q.start(chr, npc);
                }
                break;
            ...
            case 2: { // Complete Quest
                ...
                if (slea.available() >= 4) {
                    ...
                    if (chr.getDebugMessage()) {
                        chr.dropMessage(-1, "完成客戶端任務 NPC: " + npc + " Quest: " + quest);
                    }
                } else {
                    q.complete(chr, npc);
                    if (chr.getDebugMessage()) {
            ...
            case 3: { // Forefit Quest
                if (GameConstants.canForfeit(q.getId())) {
                    if (chr.getDebugMessage()) {
                        chr.dropMessage(-1, "放棄任務 Quest: " + quest);
                    }
                    q.forfeit(chr);
                } else {
                    chr.dropMessage(1, "無法放棄此任務.");
            ...
            case 4: { // Scripted Start Quest
                ...
                if (chr.getDebugMessage()) {
                    chr.dropMessage(-1, "開始腳本任務 Quest: " + quest);
                }
            ...
            case 5: { // Scripted End Quest
                ...
                if (chr.getDebugMessage()) {
                    chr.dropMessage(-1, "完成腳本任務 Quest: " + quest);
                }
                //c.getPlayer().updateTick(slea.readInt());
    ...
    public static void Storage(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (mode) {
            ...
            case 5: { // Store
                ...
                if (chr.getMeso() < 100) {
                    chr.dropMessage(1, "您的錢不足一百原來儲存物品");
                } else {
...
//                    AutobanManager.getInstance().addPoints(c, 1000, 0, "Trying to store or take out unavailable amount of mesos (" + meso + "/" + storage.getMeso() + "/" + c.getPlayer().getMeso() + ")");
            ...
            default:
                System.out.println("未處理的倉庫封包Mode : " + mode);
                break;
        }
    }
    ...
    public static void NPCMoreTalk(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (cm == null || c.getPlayer().getConversation() == 0 || (cm.getLastMsg() != lastMsg && cm.getNpc() == 9010000 && cm.getPlayer().getMapId() == 915000300)) {
