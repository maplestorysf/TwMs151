...
public class World {
        ...
        try {
            for (World wl : LoginServer.getWorlds()) {
                for (ChannelServer cs : wl.getChannels()) {
                    getStorage(0, cs.getChannel()).deregisterPendingPlayerByAccountId(accountid);
                }
            }
    ...
    public static void toggleMegaphoneMuteState(int world) {
        LoginServer.getInstance().getWorld(world).getChannels().forEach((cs) -> {
    ...
    public static void addToWheelCache(int cid, int itemId) {
        magicWheelCache.put(cid, itemId);
    }

    public static int removeFromWheelCache(int cid) {
        return ((Integer) magicWheelCache.remove(cid));
    }

    public static boolean hasWheelCache(int cid) {
        return magicWheelCache.containsKey(cid);
    }

    public static boolean isCSConnected(List<Integer> charIds) {
        return charIds.stream().anyMatch((c) -> (CashShopServer.getPlayerStorage().getPendingCharacter(c) != null));
    }

    public static boolean isCSConnected(Integer charId) {
        return CashShopServer.getPlayerStorage().getPendingCharacter(charId) != null;
    }
    ...
    public static class AutoJQ {
        ...
        public void openAutoJQ() {
            ...
            EventTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                    getAllCharacters().stream().filter((chr) -> (chr.getMapId() == 109060001)).forEachOrdered((chr) -> {
                        if (getEventMap() == 0) {
                    ...
                    EventTimer.getInstance().schedule(new Runnable() {
                        @Override
                        public void run() {
                            getAllCharacters().stream().filter((chr) -> (chr.getMapId() == getEventMap())).map((chr) -> {
    ...
    public static class Party {
        ...
        public static MapleExpedition disbandExped(int partyid) {
            ...
            if (toRemove != null) {
                removeSearch(toRemove, "隊伍已被解散.");
            }
            final MapleExpedition ret = expeds.remove(partyid);
            if (ret != null) {
                ret.getParties().forEach((p) -> {
    ...
    public static class Broadcast {
        ...
        public static void broadcastSmega(byte world, byte[] message) {
        ...
        public static void broadcastSmega(int world, byte[] message) {
            players.getAllCharacters().stream().filter((chr) -> (chr.getWorld() == world)).forEachOrdered((chr) -> {
                chr.getClient().getChannelServer().broadcastSmega(message);
            });
        }
        ...
        public static void broadcastMessage(int world, byte[] message) {
            players.getAllCharacters().stream().filter((chr) -> (chr.getWorldId() == world)).forEachOrdered((chr) -> {
                chr.announce(message);
            });
        }
            ...
            if (alliance != null) {
                for (int i = 0; i < alliance.getNoGuilds(); i++) {
                    ...
                    if (g_ == null || gid == alliance.getGuildId(i)) {
                        ...
                        guild.broadcast(CWvsContext.serverNotice(5, "[" + g_.getName() + "] 公會已經離開了聯盟."));
    ...
    public static class Family {
        ...
        public static void save() {
            System.out.println("儲存家庭中...");
            lock.writeLock().lock();
            try {
                families.values().forEach((a) -> {
                    a.writeToDB(false);
                });
            } finally {
                lock.writeLock().unlock();
    ...
    private final static int CHANNELS_PER_THREAD = 5;
    ...
    public static class Respawn implements Runnable { //is putting it here a good idea?
        ...
        public Respawn(Integer[] chs, int c) {
            StringBuilder s = new StringBuilder("[ 開起頻道 ] ");
        ...
        public void run() {
            ...
            cservs.stream().filter((cserv) -> (!cserv.hasFinishedShutdown())).forEachOrdered((cserv) -> {
                cserv.getMapFactory().getAllLoadedMaps().forEach((map) -> {
                    //iterating through each map o_x
    ...
    public static void handleMap(final MapleMap map, final int numTimes, final int size, final long now) {
        if (map.getItemsSize() > 0) {
            map.getAllItemsThreadsafe().forEach((item) -> {
        ...
        if (map.characterSize() > 0 || map.getId() == 931000500) { //jaira hack
            ...
            if (map.getMobsSize() > 0) {
                map.getAllMonstersThreadsafe().forEach((mons) -> {
                    if (mons.isAlive() && mons.shouldKill(now)) {
                        ...
                        mons.getAllBuffs().stream().filter((mse) -> (mse.shouldCancel(now))).forEachOrdered((mse) -> {
    ...
    public static void handleCooldowns(final MapleCharacter chr, final int numTimes, final boolean hurt, final long now) { //is putting it here a good idea? expensive?
        if (chr.getCooldownSize() > 0) {
            chr.getCooldowns().stream().filter((m) -> (m.startTime + m.length < now)).map((m) -> m.skillId).map((skil) -> {
        ...
        if (chr.getDiseaseSize() > 0) {
            chr.getAllDiseases().stream().filter((m) -> (m != null && m.startTime + m.length < now)).forEachOrdered((m) -> {
    ...
    public static List<ChannelServer> getAllChannels() {
        List<ChannelServer> chlist = new ArrayList<>();
    ...
    public static List<MapleCharacter> getAllCharacters() {
        ...
        LoginServer.getWorlds().forEach((worlds) -> {
            worlds.getChannels().forEach((cs) -> {
                cs.getPlayerStorage().getAllCharactersThreadSafe().forEach((chra) -> {
                    chrlist.add(chra);
                });
            });
        });
    ...
    public static void 黃色廣播() {
        WorldTimer.getInstance().registerMin(new Runnable() {
            public void run() {
                getAllChannels().forEach((cs) -> {
                    String msgs[] = {
                        "如果無法和ＮＰＣ進行對話，請使用 @ea 命令。",
                        "請不要使用粗言穢語，騷擾或詐騙其他玩家。",
                        "輸入@Help可以查看指令列表唷。",
                        "我們有獨特的尿尿系統 裝備升級唷。",
                        //"我們每天在自由市場５頻下午６~９點有大亂鬥唷，第一名獎勵豐厚唷！？",
                        "我們自由市場每掛網五分鐘有獎勵唷。",
                        "我們的ＮＸ點數獲得可以透過 活動.掛自由和贊助取得唷。",
                        "我們一開始就送背包９６格欄位唷。",
                        "ＰＶＰ頻道在五頻。",
                        "裝備勳章和沒裝備勳章的廣播有差唷。",
                        "聖誕禮物箱有機率獲得一定數量的ＮＸ唷~",
                        "弓箭手請不要使用招喚獸以免斷線。",
                        "記得隨時輸入@save唷~",
                        "勳章 <尋回初心者> 只要新手職業達到200等即可獲得",
                        "如果卡帳了請稍等3分鐘，我們每三分鐘自動解卡帳唷。"
                    };
                    int ids = (java.util.concurrent.ThreadLocalRandom.current().nextInt(msgs.length) + 1);
                    cs.yellowWorldMessage("[公告] " + msgs[ids]);
                });
            }
        }, 4, 4);
    }

    public static void 尿尿活動() {
        WorldTimer.getInstance().registerMin(new Runnable() {
            public void run() {
    ...
    public static void 在線時間獎勵() {
        WorldTimer.getInstance().registerMin(new Runnable() {
            @Override
            public void run() {
                getAllCharacters().stream().filter((chr) -> (chr.getMapId() != 910000000 && chr.getLevel() > 10)).map((chr) -> {
                    //地圖id
                    int givexp = chr.getLevel() * chr.getClient().getWorldServer().getExpRate() + chr.getLevel();
                    int giveme = chr.getLevel() * chr.getClient().getWorldServer().getMesoRate() + chr.getLevel();
                    chr.gainExp(givexp, false, false, false);
                    chr.gainMeso(giveme, true);
                    chr.dropMessage(-1, "[系統獎勵] 在線時間獲得 " + givexp + " 經驗 " + giveme + " 楓幣");
                    return chr;
                }).forEachOrdered((chr) -> {
                    chr.savePlayer();
                });
        ...
        WorldTimer.getInstance().registerMin(new Runnable() {
            ...
            public void run() {
                getAllCharacters().stream().filter((chr) -> (chr.getMapId() == 910000000 && chr.getLevel() > 10)).map((chr) -> {
                    ...
                    int givexp = chr.getLevel() * chr.getClient().getWorldServer().getExpRate() + chr.getLevel();
                    int giveme = chr.getLevel() * chr.getClient().getWorldServer().getMesoRate() + chr.getLevel();
                    int givenx = (rand(10, 15) * rand(2, 3));
                    // int givenx = (int) (random.nextInt(20) % (20 - 10 + 1) + 10) * (random.nextInt(3) % (3 - 2 + 1) + 2);
                    chr.gainExp(givexp, false, false, false);
                    chr.gainMeso(giveme, true);
                    chr.modifyCSPoints(1, givenx, false);
                    chr.dropMessage(-1, "自由掛網獲得 " + givexp + " 經驗 " + giveme + " 楓幣 " + givenx + " 點數");
                    return chr;
                }).forEachOrdered((chr) -> {
                    chr.savePlayer();
                });
    ...
    public void scheduleRateDelay(final String type, long delay) {
        WorldTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                Broadcast.broadcastMessage(getWorldId(), CWvsContext.serverMessage(rate + "雙倍活動已經結束。系統已成功自動切換為正常遊戲模式！"));
            }
        }, delay * 60 * 1000);

    }

    public static List<CheaterData> getCheaters(MapleClient c) {
        List<CheaterData> allCheaters = new ArrayList<>();
        c.getWorldServer().getChannels().forEach((cs) -> {
