...
public final class MapleFamily implements java.io.Serializable {
    ...
    public MapleFamily(final int fid) {
        ...
        try {
            ...
            members.values().stream().map((mfc) -> {
                ...
                return mfc;
            }).filter((mfc) -> (mfc.getJunior2() > 0)).forEachOrdered((mfc) -> {
    ...
    public final void broadcast(final byte[] packet, final int exceptionId, final FCOp bcop, List<Integer> cids) {
        ...
        members.values().stream().filter((mgc) -> (cids == null || cids.contains(mgc.getId()))).forEachOrdered((mgc) -> {
    ...
    public static void mergeFamily(MapleFamily newfam, MapleFamily oldfam) {
        ...
        oldfam.members.values().stream().map((mgc) -> {
