...
public class MTSCSPacket {
    ...
    public static byte[] warpCS(MapleClient c) {
        ...
        rmi.entrySet().stream().map((i) -> {
            mplew.writeInt(i.getKey()); // Item Id
            return i;
        }).filter((i) -> !(i.getKey() / 1000 != 5533)).map((i) -> {
            mplew.writeInt(i.getValue().size());
            return i;
        }).forEachOrdered((i) -> {
            i.getValue().forEach((x) -> {
                mplew.writeInt(x); // SN
            });
        });
    ...
    public static byte[] getCSInventory(MapleClient c) {
        ...
        if (mci.getItemsSize() > 0) {
            int size = 0;
            size = mci.getInventory().stream().map((itemz) -> {
                addCashItemInfo(mplew, itemz, c.getAccID(), 0);
                return itemz;
            }).filter((itemz) -> (GameConstants.isPet(itemz.getItemId()) || GameConstants.getInventoryType(itemz.getItemId()) == MapleInventoryType.EQUIP)).map((_item) -> 1).reduce(size, Integer::sum);
            mplew.writeInt(size);
            mci.getInventory().stream().filter((itemz) -> (GameConstants.isPet(itemz.getItemId()) || GameConstants.getInventoryType(itemz.getItemId()) == MapleInventoryType.EQUIP)).forEachOrdered((itemz) -> {
