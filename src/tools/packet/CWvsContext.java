...
public class CWvsContext {
    ...
    public static byte[] getAvatarMega(MapleCharacter chr, int channel, int itemId, List<String> text, boolean ear, boolean medal) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.AVATAR_MEGA.getValue());
        mplew.writeInt(itemId);
        if (medal) {
            String medalName = "<" + MapleItemInformationProvider.getInstance().getName(chr.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -49).getItemId()) + "> ";
    ...
    public static class BuffPacket {
        ...
        public static byte[] giveForeignPirate(Map<MapleBuffStat, Integer> statups, int duration, int cid, int skillid) {
            ...
            mplew.writeShort(0);
            mplew.write(0);
            statups.values().stream().map((stat) -> {
                mplew.writeInt(stat);
                return stat;
            }).map((_item) -> {
                mplew.writeLong(skillid);
        ...
        public static byte[] giveBuff(int buffid, int bufflength, Map<MapleBuffStat, Integer> statups, MapleStatEffect effect, MapleCharacter chr) {
            ...
            mplew.write(0);
            mplew.write(0);
            statups.entrySet().stream().filter((stat) -> !(!((MapleBuffStat) stat.getKey()).canStack())).map((stat) -> {
                mplew.writeInt(1);
                mplew.writeInt(buffid);
                mplew.writeInt(((Integer) stat.getValue()));
                return stat;
...
//                    List<MapleBuffStatValueHolder> buffs = chr.getStackBuffStatValueHolder(stat.getKey());
//                    if (buffs == null) {
//                        if (chr.isAdmin()) {
//                            chr.showInfo("堆疊Buff", true, "不存在Buffstats::" + stat.getKey().name());
//                        }
//                        continue;
//                    }
//                    long currentTime = 0;
//                    for (MapleBuffStatValueHolder ss : buffs) {
//                        if (ss.effect.getSourceId() == buffid) {
//                            currentTime = ss.startTime;
//                        }
//                    }
//                    mplew.writeInt(buffs.size());
//                    for (MapleBuffStatValueHolder ss : buffs) {
//                        int ctime = ((int) (currentTime - ss.startTime));
//
//                        switch (ss.effect.getSourceId()) {
//                            case 2023484: // 連續擊殺模式 - 藍色
//                            case 2023494: // 連續擊殺模式 - 紫色
//                            case 2023495: // 連續擊殺模式 - 紅色
//                                mplew.writeInt(-ss.effect.getSourceId());
//                                break;
//                            default:
//                                mplew.writeInt(ss.effect.getSourceId());
//                        }
//
//                        mplew.writeInt(ss.value);
//                        mplew.writeInt(((int) currentTime) - ctime);
//                        mplew.writeInt(ctime);
//                        mplew.writeInt(ss.localDuration);
//                    }
//                }
//            }
//            for (Map.Entry stat : statups.entrySet()) {
...
//                mplew.writeInt(bufflength);
//            }
            if ((statups.containsKey(MapleBuffStat.DAMAGE_BUFF)) && (statups.containsKey(MapleBuffStat.MP_BUFF))
                    && (statups.containsKey(MapleBuffStat.CRITICAL_RATE_BUFF)) && (statups.containsKey(MapleBuffStat.DAMAGE_TAKEN_BUFF))
                    && (statups.containsKey(MapleBuffStat.DODGE_CHANGE_BUFF))) {
                mplew.write(0);
        ...
        public static byte[] cancelBuff(List<MapleBuffStat> statups, MapleCharacter chr) {
            ...
            statups.stream().filter((stat) -> (stat.canStack() && !GameConstants.isSpecialStackBuff(stat))).map((stat) -> chr.getStackBuffStatValueHolder(stat)).filter((buffs) -> !(buffs == null)).forEachOrdered((buffs) -> {
//                    System.out.println("堆疊Buff::" + stat.name()); // 查看排列順序用
            ...
            statups.stream().filter((z) -> (z == MapleBuffStat.MONSTER_RIDING)).forEachOrdered((_item) -> {
...
//            mplew.write(1);
    ...
    public static class InfoPacket {
        ...
        public static byte[] updateQuestMobKills(final MapleQuestStatus status) {
            ...
            status.getMobKills().values().forEach((kills) -> {
    ...
    public static class GuildPacket {
        ...
        public static byte[] showThread(MapleBBSThread thread) {
            ...
            thread.replies.values().stream().map((reply) -> {
    ...
    public static class PartyPacket {
        ...
        public static void addPartyStatus(int forchannel, MapleParty party, MaplePacketLittleEndianWriter lew, boolean leaving, boolean exped) {
            ...
            partymembers.forEach((partychar) -> {
    ...
    public static byte[] updatePlayerStats(final Map<MapleStat, Integer> mystats, final boolean itemReaction, final MapleCharacter chr) {
        ...
        long updateMask = 0;
        updateMask = mystats.keySet().stream().map((statupdate) -> statupdate.getValue()).reduce(updateMask, (accumulator, _item) -> accumulator | _item);
    ...
    public static byte[] charInfo(final MapleCharacter chr, final boolean isSelf) {
        ...
        chr.getInventory(MapleInventoryType.SETUP).newList().stream().filter((i) -> (i.getItemId() / 10000 == 301 && !chairs.contains(i.getItemId()))).forEachOrdered((i) -> {
    ...
    public static byte[] Mulung_Pts(int recv, int total) {
        ...
        return showQuestMsg(new StringBuilder().append("您已收到 ").append(recv).append(" 訓練點，共累計共").append(total).append(" 訓練點.").toString());
    }
    ...
    private static byte[] serverMessage(int type, int channel, String message, boolean megaEar) {
        return serverMessage(type, channel, message, megaEar, false);
    }

    private static byte[] serverMessage(int type, int channel, String message, boolean megaEar, boolean medal) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] getOwlSearched(final int itemSearch, final List<HiredMerchant> hms) {
        ...
        size = hms.stream().map((hm) -> hm.searchItem(itemSearch).size()).reduce(size, Integer::sum);
        mplew.writeInt(size);
    ...
    public static byte[] getFamiliarInfo(MapleCharacter chr) {
        ...
        chr.getInventory(MapleInventoryType.USE).list().stream().filter((i) -> (i.getItemId() / 10000 == 287)).forEachOrdered((i) -> {
            //expensif
    ...
    public static byte[] fairyPendantMessage(final int termStart, final int incExpR) { // bonusExp, <= 0x3D
        ...
        mplew.writeInt(0); // hour = 0 upon equipping
