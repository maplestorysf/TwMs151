                ...
                t1 = new Thread() {
                    ...
                    public void run() {
                        try {
                            ...
                            Logger.getLogger(DumpQuests.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }
                };
                t1.start();
...
//            ps.setString(4, MapleDataTool.getString("desc", stringData, ""));
        ...
        if (id != 4031454 && id != 4310008 && MapleDataTool.getIntConvert("info/tradeBlock", iz, 0) > 0) {
//        if (id != 4310008 && MapleDataTool.getIntConvert("info/tradeBlock", iz, 0) > 0) {
        ...
        if (dat != null) {
            ...
            for (MapleData d : dat.getChildren()) {
                ...
                if (z != null) {
                    Pair<Integer, Integer> incs;
                    if (z.isElement()) {
                        String ele = MapleDataTool.getString(z.getValue1(), d, "F30"); //value1=value2
                        incs = new Pair<>(Element.getFromChar(ele.charAt(0)).getValue(), Integer.parseInt(ele.substring(1, ele.length()))); //E.G. F30
                    } else {
                        incs = new Pair<>(MapleDataTool.getIntConvert(z.getValue1(), d, 0), MapleDataTool.getIntConvert(z.getValue2(), d, 0));
                    }
    ...
    public void dumpItems(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse) throws Exception {
        if (!update) {
            ...
            System.out.println("刪除資料表 wz_itemdata 成功.");
            delete("DELETE FROM wz_itemequipdata");
            System.out.println("刪除資料表 wz_itemequipdata 成功.");
            delete("DELETE FROM wz_itemadddata");
            System.out.println("刪除資料表 wz_itemadddata 成功.");
            delete("DELETE FROM wz_itemrewarddata");
            System.out.println("刪除資料表 wz_itemrewarddata 成功.");
        }

        System.out.println("開始讀取 xml...中");
        dumpItems(item, psa, psr, ps, pse, false);
        dumpItems(character, psa, psr, ps, pse, true);
        System.out.println("添加資料至資料庫 wz_itemdata 中.....");
    ...
    public static void main(String[] args) {
        ...
        System.out.println("添加資料完成 " + withErrors + " 使用了 " + elapsedMinutes + " 分 " + elapsedSecs + " 秒");
    }
