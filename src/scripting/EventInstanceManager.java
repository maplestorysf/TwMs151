...
public class EventInstanceManager {
    ...
    private boolean unregisterPlayer_NoLock(final MapleCharacter chr) {
        if (name.equals("CWKPQ")) { //hard code it because i said so
            LoginServer.getWorlds().stream().map((world) -> ChannelServer.getInstance(world.getWorldId(), channel).getMapleSquad("CWKPQ")).filter((squad) -> (squad != null)).map((squad) -> {
                squad.removeMember(chr.getName());
                return squad;
            }).filter((squad) -> (squad.getLeaderName().equals(chr.getName()))).forEachOrdered((_item) -> {
