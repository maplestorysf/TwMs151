...
//    private List<Pair<Integer, Integer>> randomMorph;
    ...
    private static MapleStatEffect loadFromData(final MapleData source, final int sourceid, final boolean skill, final boolean overTime, final int level, final String variables) {
        ...
        if (ret.overTime && ret.getSummonMovementType() == null && !ret.isEnergyCharge()) {
            ...
            //addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.DARKSIGHT, Integer.valueOf(ret.x));
        ...
        if (ret.skill) { // hack because we can't get from the datafile...
            switch (sourceid) {
                ...
                case 23111004:
                    ...
                    break;
                case 80001079:
                    ret.statups.put(MapleBuffStat.WATK, Integer.valueOf(ret.damage));
                    ret.statups.put(MapleBuffStat.MATK, Integer.valueOf(ret.damage));
                    ret.duration = 30 * 1000;
                    break;
                case 80001080:
                    ret.statups.put(MapleBuffStat.WDEF, ret.x);
                    ret.statups.put(MapleBuffStat.MDEF, ret.y);
                    ret.duration = 30 * 1000;
                    break;
                case 80001081:
                    ret.statups.put(MapleBuffStat.EXPRATE, ret.x);
                    ret.duration = 30 * 1000;
                    break;
                ...
                case 51101004:
                    ...
                    //          ret.statups.put(MapleBuffStat., 30);
                    break;

                case 24121004:
                    ret.statups.put(MapleBuffStat.ENHANCED_WATK, 30);
                    //          ret.statups.put(MapleBuffStat., 30);
                    break;
                case 5121015:
                    ret.statups.put(MapleBuffStat.ENHANCED_WATK, 15);
                    ret.statups.put(MapleBuffStat.ABNORMAL_STATUS_R, 30);
                    //          ret.statups.put(MapleBuffStat., 30);
                    break;

                case 51111004:
                    ret.statups.put(MapleBuffStat.DEFENCE_BOOST_R, 100);
                    ret.statups.put(MapleBuffStat.ELEMENTAL_STATUS_R, 60);
                    ret.statups.put(MapleBuffStat.ABNORMAL_STATUS_R, 60);
                    //          ret.statups.put(MapleBuffStat., 30);
                    break;
                case 51121006:
                    ret.statups.put(MapleBuffStat.ENHANCED_WATK, 60);
                    ret.statups.put(MapleBuffStat.CRITICAL_RATE_BUFF, 30);
                    //          ret.statups.put(MapleBuffStat., 30);
                    break;
                case 5111010:
                    ret.statups.put(MapleBuffStat.DEFENCE_BOOST_R, 15);
                    //          ret.statups.put(MapleBuffStat., 30);
                    break;
                case 4211008:
                ...
                case 2120010:
                    ret.duration = 5000;
                    ret.statups.put(MapleBuffStat.MANY_USES, ret.x);
                    break;
    ...
    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, int newDuration) {
        ...
        if (!applyfrom.isGM()) {
            if (skill && (sourceid == 9001000 || sourceid == 9001001 || sourceid == 9001002 || sourceid == 9101000 || sourceid == 9101001 || sourceid == 9101002 || sourceid == 9101003 || sourceid == 9101004 || sourceid == 9101005 || sourceid == 9101006 || sourceid == 9101007 || sourceid == 9101008)) {
                World.Broadcast.broadcastMessage(applyfrom.getWorld(), CWvsContext.serverNotice(6, "[自動封鎖]: " + applyfrom.getName() + " 修改封包使用GM技能."));
                applyfrom.ban("修改封包使用GM技能!", true);
                return false;
            }
        }
        ...
        if (expinc != 0) {
            ...
            for (int i = 0; i <= use.getSlotLimit(); i++) { // impose order...
                ...
                if (item != null) {
                    if (ItemConstants.isThrowingStar(item.getItemId()) && item.getQuantity() >= 100 || ItemConstants.isBullet(item.getItemId()) && item.getQuantity() >= 100) {
            ...
            applyto.getParty().getMembers().stream().filter((mpc) -> (mpc.getId() != applyto.getId() && mpc.getChannel() == applyto.getClient().getChannel() && mpc.getMapid() == applyto.getMapId() && mpc.isOnline())).map((mpc) -> applyto.getMap().getCharacterById(mpc.getId())).filter((mc) -> (mc != null)).forEachOrdered((mc) -> {
            ...
            if (typeee > 0 || effectedOnEnemy > 0) {
                applyto.getMap().getCharactersThreadsafe().stream().filter((chr) -> (chr.getId() != applyto.getId() && (effectedOnAlly > 0 ? (chr.getTeam() == applyto.getTeam()) : (chr.getTeam() != applyto.getTeam() || typeee == 0)))).forEachOrdered((chr) -> {
            ...
            if (effectedOnEnemy > 0) {
                ...
                applyto.getMap().getCharactersThreadsafe().stream().filter((chr) -> (chr.getId() != applyto.getId() && (chr.getTeam() != applyto.getTeam() || ffsa == 0))).forEachOrdered((chr) -> {
        ...
        if (summonMovementType != null && (sourceid != 32111006 || (applyfrom.getBuffedValue(MapleBuffStat.REAPER) != null && !primary))) {
            ...
            if (isBeholder()) {
                ...
                try {
                    ss.stream().filter((s) -> (s.getSkill() == sourceid)).forEachOrdered((s) -> {
        ...
        if (isMagicDoor()) { // Magic Door
            ...
            applyto.getCooldowns().stream().filter((i) -> (i.skillId != 5121010)).map((i) -> {
        ...
        if (rewardItem != null && totalprob > 0) {
            rewardItem.stream().filter((reward) -> (MapleInventoryManipulator.checkSpace(applyto.getClient(), reward.left, reward.mid, "") && reward.right > 0 && Randomizer.nextInt(totalprob) < reward.right)).forEachOrdered((reward) -> {
                // Total prob
        ...
        if (familiarTarget == 2 && applyfrom.getParty() != null && primary) { //to party
            applyfrom.getParty().getMembers().stream().filter((mpc) -> (mpc.getId() != applyfrom.getId() && mpc.getChannel() == applyfrom.getClient().getChannel() && mpc.getMapid() == applyfrom.getMapId() && mpc.isOnline())).map((mpc) -> applyfrom.getMap().getCharacterById(mpc.getId())).filter((mc) -> (mc != null)).forEachOrdered((mc) -> {
                applyTo(applyfrom, mc, false, null, newDuration);
            });
        } else if (familiarTarget == 3 && primary) {
            applyfrom.getMap().getCharactersThreadsafe().stream().filter((mc) -> (mc.getId() != applyfrom.getId())).forEachOrdered((mc) -> {
                applyTo(applyfrom, mc, false, null, newDuration);
            });
    ...
    private void applyBuff(final MapleCharacter applyfrom, int newDuration) {
        if (isSoulStone()) {
            if (applyfrom.getParty() != null) {
                int membrs = 0;
                membrs = applyfrom.getMap().getCharactersThreadsafe().stream().filter((chr) -> (chr.getParty() != null && chr.getParty().getId() == applyfrom.getParty().getId() && chr.isAlive())).map((_item) -> 1).reduce(membrs, Integer::sum);
                List<MapleCharacter> awarded = new ArrayList<>();
                while (awarded.size() < Math.min(membrs, y)) {
                    applyfrom.getMap().getCharactersThreadsafe().stream().filter((chr) -> (chr != null && chr.isAlive() && chr.getParty() != null && chr.getParty().getId() == applyfrom.getParty().getId() && !awarded.contains(chr) && Randomizer.nextInt(y) == 0)).forEachOrdered((chr) -> {
                        awarded.add(chr);
                    });
                }
                awarded.stream().map((chr) -> {
            ...
            affecteds.stream().map((affectedmo) -> (MapleCharacter) affectedmo).filter((affected) -> (affected.getId() != applyfrom.getId() && (isGmBuff() || (applyfrom.inPVP() && affected.getTeam() == applyfrom.getTeam() && Integer.parseInt(applyfrom.getEventInstance().getProperty("type")) != 0) || (applyfrom.getParty() != null && affected.getParty() != null && applyfrom.getParty().getId() == affected.getParty().getId())))).map((affected) -> {
                ...
                return affected;
            }).filter((affected) -> (isTimeLeap())).forEachOrdered((affected) -> {
                affected.getCooldowns().stream().filter((i) -> (i.skillId != 5121010)).map((i) -> {
    ...
    public final void silentApplyBuff(final MapleCharacter chr, final long starttime, final int localDuration, final Map<MapleBuffStat, Integer> statup, final int cid) {
        chr.registerEffect(this, starttime, BuffTimer.getInstance().schedule(new CancelEffectAction(chr, this, starttime, statup), ((starttime + localDuration) - System.currentTimeMillis())), statup, true, localDuration, cid);
    ...
    public final void applyComboBuff(final MapleCharacter applyto, short combo) {
        ...
        applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid, 99999, stat, this, applyto)); // Hackish timing, todo find out
...
//	final ScheduledFuture<?> schedule = TimerManager.getInstance().schedule(cancelAction, ((starttime + 99999) - System.currentTimeMillis()));
    ...
    private void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final int newDuration) {
        ...
        switch (sourceid) {
            ...
            case 5320007: {//dice
                ...
                if (buffid >= 100) {
                    applyto.dropMessage(-6, "[雙倍幸運骰子] 擲出了兩個相同數字 (" + (buffid / 100) + ")");
                } else if (buffid >= 10) {
                    applyto.dropMessage(-6, "[雙倍幸運骰子] 擲出了兩顆骰子 (" + (buffid / 10) + "、" + (buffid % 10) + ")");
                }
                localstatups = new EnumMap<>(MapleBuffStat.class);
            ...
            case 32111005: { //body boost
                ...
                applyto.getClient().getSession().write(BuffPacket.giveBuff(sourcez, localDuration, stat, this, applyto));
                normal = false;
                break;
            }
            case 32001003: {//dark aura
            ...
            case 32120000: { // adv dark aura
                ...
                applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid == 32120000 ? 32001003 : sourceid, localDuration, statt, this, applyto));
                statt.clear();
                statt.put(MapleBuffStat.DARK_AURA, x);
                applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid, localDuration, statt, this, applyto));
            ...
            case 32110000: { // advanced blue aura
                ...
                applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid == 32110000 ? 32111012 : sourceid, localDuration, statt, this, applyto));
                statt.clear();
            ...
            case 32120001: { // advanced yellow aura
                ...
                applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid == 32120001 ? 32101003 : sourceid, localDuration, statt, this, applyto));
                statt.clear();
    ...
    private boolean isMist() {
        return skill && (sourceid == 2111003 || sourceid == 12111005 || sourceid == 14111006 || sourceid == 22161003 || sourceid == 32121006 || sourceid == 1076 || sourceid == 11076); // poison mist, smokescreen and flame gear, recovery aura
    }
    ...
    public final boolean isUnstealable() {
        if (statups.keySet().stream().anyMatch((b) -> (b == MapleBuffStat.MAPLE_WARRIOR))) {
            return true;
        }
