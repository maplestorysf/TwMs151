...
public class MapleItemInformationProvider {
                ...
                if (level.getType() != MapleDataType.INT) {
                    for (MapleData leve : level) {
                        if (!leve.getName().equals("representName") && !leve.getName().equals("typeName")) {
                            ...
                            itemz.itemIDs.add(MapleDataTool.getString(level));//itemIDs轉String
                        }
                    }
                } else {
                    String s = Integer.toString(MapleDataTool.getInt(level)); //轉String後不支援直接讀取int故轉回int改用String方式讀取
                    itemz.itemIDs.add(s);
                    //  itemz.itemIDs.add(MapleDataTool.getInt(level));
                }
    ...
    public void runItems() {
        ...
        try {
            ...
            dataCache.entrySet().stream().filter((entry) -> (GameConstants.getInventoryType(entry.getKey()) == MapleInventoryType.EQUIP)).forEachOrdered((entry) -> {
    ...
    public boolean canEquip(Map<String, Integer> stats, int itemid, int level, int job, int fame, int str, int dex, int luk, int int_) {
        if (level >= (stats.containsKey("reqLevel") ? stats.get("reqLevel") : 0)
    ...
    public Item getEquipById_2(int equipId) {
        return getEquipById_2(equipId, -1);
    }

    public Item getEquipById_2(int equipId, int ringId) {
    ...
    public boolean isMSI(Equip equip, short stat) {
        return equip.getStr() > stat && equip.getDex() > stat && equip.getInt() > stat && equip.getLuk() > stat && equip.getMatk() > stat && equip.getWatk() > stat && equip.getAcc() > stat && equip.getAvoid() > stat && equip.getSpeed() > stat && equip.getJump() > stat && equip.getWdef() > stat && equip.getMdef() > stat && equip.getMp() > stat && equip.getHp() > stat;
    }
    ...
    public Equip levelUpEquipCustom(Equip equip, int levelTo) {
        ...
        if (levelTo <= 10) {
            ...
            PADGain = rng.nextInt(5);
            MADGain = rng.nextInt(5);
            PDDGain = rng.nextInt(5);
            MDDGain = rng.nextInt(5);
            accGain = rng.nextInt(5);
            HPGain = rng.nextInt(21);
            MPGain = rng.nextInt(21);
        } else if (levelTo <= 20) {
            statGain = rng.nextInt(3);
            PADGain = rng.nextInt(10);
            MADGain = rng.nextInt(10);
            PDDGain = rng.nextInt(10);
            MDDGain = rng.nextInt(10);
            accGain = rng.nextInt(10);
            HPGain = rng.nextInt(26);
            MPGain = rng.nextInt(26);
        } else if (levelTo <= 30) {
            statGain = rng.nextInt(4);
            PADGain = rng.nextInt(15);
            MADGain = rng.nextInt(15);
            PDDGain = rng.nextInt(15);
            MDDGain = rng.nextInt(15);
            accGain = rng.nextInt(15);
            HPGain = rng.nextInt(31);
            MPGain = rng.nextInt(31);
        } else if (levelTo <= 40) {
            statGain = rng.nextInt(5);
            PADGain = rng.nextInt(20);
            MADGain = rng.nextInt(20);
            PDDGain = rng.nextInt(20);
            MDDGain = rng.nextInt(20);
            accGain = rng.nextInt(20);
            HPGain = rng.nextInt(51);
            MPGain = rng.nextInt(51);
        } else if (levelTo <= 49) {
            statGain = rng.nextInt(6);
            PADGain = rng.nextInt(25);
            MADGain = rng.nextInt(25);
            PDDGain = rng.nextInt(25);
            MDDGain = rng.nextInt(25);
            accGain = rng.nextInt(25);
            HPGain = rng.nextInt(111);
            MPGain = rng.nextInt(111);
        } else if (levelTo == 50) {
            statGain = 50;
            PADGain = 50;
            MADGain = 50;
        ...
        if (levelTo == 50) {
            Equip n = nEquip;
            nEquip.setStr(n.getStr() + statGain);
            nEquip.setDex(n.getDex() + statGain);
            nEquip.setInt(n.getInt() + statGain);
            nEquip.setLuk(n.getLuk() + statGain);
            nEquip.setWatk(n.getWatk() + PADGain);
            nEquip.setMatk(n.getMatk() + MADGain);
            nEquip.setWdef(n.getWdef() + PDDGain);
            nEquip.setMdef(n.getMdef() + MDDGain);
            nEquip.setHp(n.getHp() + HPGain);
            nEquip.setMp(n.getMp() + MPGain);
            nEquip.setAcc(n.getAcc() + accGain);
            nEquip.setAvoid(n.getAvoid() + accGain);
        } else {
            statGain += 1;
            PADGain += 1;
            MADGain += 1;
