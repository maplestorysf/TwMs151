...
public class MapleOxQuiz extends MapleEvent {
    ...
    public void sendQuestion(final MapleMap toSend) {
        ...
        oxSchedule2 = EventTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                int number = 0;
                number = toSend.getCharactersThreadsafe().stream().filter((mc) -> (mc.isGM() || !mc.isAlive())).map((_item) -> 1).reduce(number, Integer::sum);
                if (toSend.getCharactersSize() - number <= 1 || timesAsked == 10) {
                    toSend.broadcastMessage(CWvsContext.serverNotice(6, "活動已經結束"));
                    unreset();
                    toSend.getCharactersThreadsafe().stream().filter((chr) -> (chr != null && !chr.isGM() && chr.isAlive())).map((chr) -> {
                        chr.canTalk(true);
                        return chr;
                    }).map((chr) -> {
                        givePrize(chr);
        ...
        oxSchedule = EventTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                timesAsked++;
                toSend.getCharactersThreadsafe().stream().filter((chr) -> (chr != null && !chr.isGM() && chr.isAlive())).forEachOrdered((chr) -> {
