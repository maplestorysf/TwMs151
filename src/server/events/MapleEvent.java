...
public abstract class MapleEvent {
                ...
                if (e.isRunning) {
                    for (int i : e.type.mapids) {
                        if (cserv.getEvent() == i) {
                            ...
                            e.broadcast(CWvsContext.serverNotice(0, "活動會在30秒後開始!"));
    ...
    public static String scheduleEvent(final MapleEventType event, final ChannelServer cserv) {
        ...
        World.Broadcast.broadcastMessage(worldSt, CWvsContext.serverNotice(0, "我們舉辦個活動 " + StringUtil.makeEnumHumanReadable(event.name()) + "  在頻道 " + cserv.getChannel() + "! 換到頻道 " + cserv.getChannel() + " 和使用 @event 指令!"));
        return "";
    }
}
