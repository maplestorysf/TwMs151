...
public class InactiveAccountDeleterWindow extends javax.swing.JFrame {
    ...
    private void DeleteStartActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (deleteConfirmed) {
            ...
            int totAccs = 0;
            totAccs = accountIDLists.stream().map((a) -> a.size()).reduce(totAccs, Integer::sum);
    ...
    public synchronized void write(byte[] ba, int str, int len) {
        try {
            ...
            if (bytesEndWith(ba, str, len, LINE_SEP)) {
                lineLengths.addLast(curLength);
                curLength = 0;
                if (lineLengths.size() > maxLines) {
                    textArea.replaceRange(null, 0, ((Integer) lineLengths.removeFirst()));
                }
            }
