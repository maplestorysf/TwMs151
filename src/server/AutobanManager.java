...
public class AutobanManager implements Runnable {
    ...
    public final void autobans(final MapleClient c, final String reason) {
        if (c.getPlayer() == null) {
            return;
        }
        if (c.getPlayer().isGM()) {
        ...
        addPointss(c, AUTOBAN_POINTS, 0, reason);
    }

    public final void addPointss(final MapleClient c, final int points, final long expiration, final String reason) {
        lock.lock();
        try {
            ...
            if (this.points.get(acc) >= AUTOBAN_POINTS) { // See if it's sufficient to auto ban
                ...
                sb.append("): ");
                reasons.get(acc).stream().map((s) -> {
                    sb.append(s);
                    return s;
                }).forEachOrdered((_item) -> {
                    sb.append(", ");
                });
                World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "[自動封鎖] " + c.getPlayer().getName() + " 因為使用違法程式練功而永久停權。"));// (Last reason: " + reason + ")
