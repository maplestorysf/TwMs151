...
public class MapleSquad {
    ...
    public static enum MapleSquadType {
        ...
        bossbalrog(2), zak(2), chaoszak(3), horntail(2), chaosht(3), pinkbean(3), nmm_squad(2), vergamot(2), dunas(2), nibergen_squad(2), dunas2(2), core_blaze(2), aufheben(2), cwkpq(10), tokyo_2095(2), vonleon(3), scartar(2), cygnus(3), hilla(1), arkarium(3), bloodyMary(3), ch_bloodyMary(3);
            ...
            if (theirCh > 0) {
                ...
                if (lead != null && lead.getMapId() == beginMapId && lead.getClient().getChannel() == ch) {
                    ...
                    if (ChannelServer.getInstance(wl, ch).addMapleSquad(squad, type.name())) {
                        ...
                        type.queuedPlayers.get(ch).add(new Pair<String, String>(nextPlayerId, " 成功"));
                        //	type.queuedPlayers.get(ch).add(new Pair<>(nextPlayerId, "Success"));
                    } else {
                        squad.clear();
                        type.queuedPlayers.get(ch).add(new Pair<String, String>(nextPlayerId, " 忽略"));
                        //type.queuedPlayers.get(ch).add(new Pair<>(nextPlayerId, "Skipped"));
                    }
                    break;
                } else {
                    if (lead != null) {
                        lead.dropMessage(6, "你的遠征隊因為在錯誤的地圖而被略過");
                        //lead.dropMessage(6, "Your squad has been skipped due to you not being in the right channel and map.");
                    }
                    getBeginMap().broadcastMessage(CWvsContext.serverNotice(6, nextPlayerId + " 的遠征隊因為在錯誤的地圖而被略過"));
                    type.queuedPlayers.get(ch).add(new Pair<String, String>(nextPlayerId, " 不在地圖上"));

                }
            } else {
                getBeginMap().broadcastMessage(CWvsContext.serverNotice(6, nextPlayerId + " 的遠征隊因為玩家不在線上而被略過"));
                type.queuedPlayers.get(ch).add(new Pair<String, String>(nextPlayerId, " 不在線上"));
    ...
    public MapleMap getBeginMap() {
        return ChannelServer.getInstance(wl, ch).getMapFactory().getMap(beginMapId);
    }
    ...
    public void clear() {
        ...
        bannedMembers.clear();
        leader = null;
        ChannelServer.getInstance(wl, ch).removeMapleSquad(type);
        this.status = 0;
    }

    public MapleCharacter getChar(String name) {
        return ChannelServer.getInstance(wl, ch).getPlayerStorage().getCharacterByName(name);
    }
    ...
    public String getNextPlayer() {
        ...
        sb.append("#b").append(type.queue.get(ch).size()).append(" #k ").append("隊員列表：\n\r ");
        int i = 0;
    ...
    public boolean containsMember(MapleCharacter member) {
        return members.keySet().stream().anyMatch((mmbr) -> (mmbr.equalsIgnoreCase(member.getName())));
    }
        ...
        if (join) {
            if (!containsMember(member) && !getAllNextPlayer().contains(member.getName())) {
                if (members.size() <= 30) {
                    ...
                    getLeader().dropMessage(5, member.getName() + " (" + job + ") 加入了遠征隊");
            ...
            if (containsMember(member)) {
                ...
                getLeader().dropMessage(5, member.getName() + " (" + job + ") 離開了遠征隊");
    ...
    public void acceptMember(int pos) {
        ...
        if (toadd != null && getChar(toadd) != null) {
            ...
            getChar(toadd).dropMessage(5, getLeaderName() + " 已讓你返回遠征隊");
    ...
    public void banMember(int pos) {
        ...
        if (toban != null && getChar(toban) != null) {
            ...
            getChar(toban).dropMessage(5, getLeaderName() + " 把你從遠征隊移除了");
    ...
    public String getSquadMemberString(byte type) {
        switch (type) {
            case 0: {
                StringBuilder sb = new StringBuilder("遠征隊人數：");
                sb.append("#b").append(members.size()).append(" #k ").append("隊員列表：\n\r ");
                int i = 0;
