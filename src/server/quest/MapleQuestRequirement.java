...
public class MapleQuestRequirement implements Serializable {
    ...
    public boolean check(MapleCharacter c, Integer npcid) {
        switch (type) {
            case job:
                if (dataStore.stream().anyMatch((a) -> (a.getRight() == c.getJob() || c.isGM()))) {
                    return true;
                }
                return false;
            case skill: {
            ...
            case pet:
                if (dataStore.stream().anyMatch((a) -> (c.getPetById(a.getRight()) != -1))) {
                    return true;
                }
                return false;
            case pettamenessmin:
                if (c.getPets().stream().anyMatch((pet) -> (pet.getSummoned() && pet.getCloseness() >= intStore))) {
                    return true;
                }
                return false;
            case partyQuest_S:
