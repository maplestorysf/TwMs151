...
public class ChangeableStats extends OverrideMonsterStats {
    ...
    public ChangeableStats(MapleMonsterStats stats, int newLevel, int multipler) {
        final double base = ((double) newLevel / (double) stats.getLevel()) * multipler;
        hp = (int) (stats.getHp() * base);
        ...
        acc = (int) Math.round(stats.getAcc() + base);
        eva = (int) Math.round(stats.getEva() + base);
        PDRate = Math.min(stats.isBoss() ? 30 : 10, (int) Math.round(stats.getPDRate() * base));
        MDRate = Math.min(stats.isBoss() ? 30 : 10, (int) Math.round(stats.getMDRate() * base));
        pushed = (int) Math.round(stats.getPushed() * base);
        level = newLevel > 250 ? 250 : newLevel;//change the 250 to 200 if too hard.
    }
