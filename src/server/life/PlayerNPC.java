...
public class PlayerNPC extends MapleNPC implements MapleCharacterLook {
    ...
    public static void updateByCharId(MapleCharacter chr) {
        if (World.Find.findChannel(chr.getId()) > 0) { //if character is in cserv
            ChannelServer.getInstance(World.Find.findWorld(chr.getId()), World.Find.findChannel(chr.getId())).getAllPlayerNPC().forEach((npc) -> {
                npc.update(chr);
            });
        }
    }
    ...
    public void addToServer() {
        LoginServer.getWorlds().forEach((worlds) -> {
            worlds.getChannels().forEach((cserv) -> {
    ...
    public void update(MapleCharacter chr) {
        ...
        chr.getInventory(MapleInventoryType.EQUIPPED).newList().stream().filter((item) -> !(item.getPosition() < -127)).forEachOrdered((item) -> {
