...
public class SpawnPoint extends Spawns {
    ...
    public final MapleMonster spawnMonster(final MapleMap map) {
        ...
        if (carnivalTeam > -1) {
            map.getAllReactorsThreadsafe().stream().filter((r) -> (r.getName().startsWith(String.valueOf(carnivalTeam)) && r.getReactorId() == (9980000 + carnivalTeam) && r.getState() < 5)).map((r) -> Integer.parseInt(r.getName().substring(1, 2))).map((num) -> MapleCarnivalFactory.getInstance().getGuardian(num)).filter((skil) -> (skil != null)).forEachOrdered((skil) -> {
        ...
        for (MapleSummon s : map.getAllSummonsThreadsafe()) {
            if (s.getSkill() == 35111005) {
                ...
                effect.getMonsterStati().entrySet().forEach((stat) -> {
