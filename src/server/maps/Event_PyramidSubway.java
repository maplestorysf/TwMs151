...
public class Event_PyramidSubway {
    ...
    public final void commenceTimerNextMap(final MapleCharacter c, final int stage) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            c.getParty().getMembers().stream().map((mpc) -> ourMap.getCharacterById(mpc.getId())).filter((chr) -> (chr != null)).map((chr) -> {
    ...
    public final void succeed(final MapleCharacter c) {
        ...
        int exp = 0;
        if (rank < 4) {
            exp = ((kill + (cool * 5)) + pt) * c.getClient().getWorldServer().getExpRate();
    ...
    private static final void changeMap(final MapleCharacter c, final MapleMap map, final int minLevel, final int maxLevel, final int clear) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            c.getParty().getMembers().stream().map((mpc) -> oldMap.getCharacterById(mpc.getId())).filter((chr) -> (chr != null && chr.getId() != c.getId() && chr.getLevel() >= minLevel && chr.getLevel() <= maxLevel)).map((chr) -> {
                if (clear == 1) {
