...
public abstract class Timer {
    ...
    public void start() {
        ...
        file = "logs/異常/Log_" + name + "_Except.rtf";
    ...
    public ScheduledFuture<?> registerMin(Runnable r, long repeatTime, long delay) {
        if (ses == null) {
            return null;
        }
        return ses.scheduleAtFixedRate(new LoggingSaveRunnable(r, file), delay, repeatTime, TimeUnit.MINUTES);
    }

    public ScheduledFuture<?> registerDay(Runnable r, long repeatTime, long delay) {
        if (ses == null) {
            return null;
        }
        return ses.scheduleAtFixedRate(new LoggingSaveRunnable(r, file), delay, repeatTime, TimeUnit.DAYS);
    }
